...
const searchSongs = async (query, searchType) => {
...
// 가사 가져오기 (백엔드 프록시 호출) — 에러 본문 로그 추가
const fetchLyrics = async (trackName, artistName) => {
  const params = new URLSearchParams({ t: trackName, a: artistName });
  const res = await fetch(`/api/lyrics?${params.toString()}`);
  const text = await res.text();

  if (!res.ok) {
    // 여기서 원문 에러 메시지 확인 가능 (쿼터/파라미터 오류 등)
    console.log("lyrics error body:", text);
    throw new Error(`가사 조회 실패 (${res.status})`);
  }
...
const TrackSearchwithLyrics = () => {
  ...
  const onSelect = async (item) => {
    const track = item.usTrack || item.krTrack || item; // 가능한 영어(US) 데이터 우선
    setError("");
    setLoading(true);
...
export default TrackSearchwithLyrics;
