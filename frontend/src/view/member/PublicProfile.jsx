...
const imgUrl = (p) =>
    !p ? "" : /^https?:\/\//i.test(p) ? p : `${API_HOST}/${String(p).replace(/^\/+/, "")}`;
...
const PublicProfile = () => {
  ...
  useEffect(() => {
    axios
        .get(`/api/users/${id}/profile`, { withCredentials: true })
    ...
    axios
        ...
        .then((res) => {
          const mapped = (res.data || []).map((album) => ({
            ...
            title: album.title,
            artist: Array.isArray(album.artists) ? album.artists.map((a) => a.name).join(", ") : "",
          }));
          setLikedAll(mapped);
        })
  ...
  const avatarUrl = profile.avatar ? imgUrl(profile.avatar) : defaultProfile;
            ...
            <ModalBody maxH="80vh" overflowY="auto" sx={{ overscrollBehaviorY: "contain" }}>
              <SimpleGrid columns={[2, 3, 4]} spacing={8} py={4}>
                {likedAll.map((album) => (
                    ...
                    >
                      <Image
                          src={album.imagePath ? imgUrl(album.imagePath) : ""}
                          alt={album.title}
                          w="160px"
                          h="160px"
                          objectFit="cover"
